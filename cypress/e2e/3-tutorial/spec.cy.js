...
describe("testando alura pic", () => {
  ...
  const credenciais = require("./data/usuarios.json");

  credenciais.forEach((credencial) => {
    it.only("deve printar as credenciais", () => {
      const { email, password } = credencial;
      cy.get('input[formcontrolname="userName"]').type(email).tab();
      cy.get('input[formcontrolname="password"]').type(password).tab();
